...
def get_embeddings_tokens(tokens, sep, tokenizer, model, cased=True, emb_start = 0, emb_end = None, is_seq2seq = False):
    if is_seq2seq:
...
all_results = []
for layernum in tqdm(range(25), total = 25):
...
plt.figure(figsize=(5, 4), dpi=300)
plt.barh(best_l['language'], best_l['r_random'], color='gray', label='r_transf')
plt.axvline(0, color="black", lw=1)
plt.barh("Control", best_l['r_tot'].mean(), color='tomato', label='Total R')
plt.xlabel('R')
plt.gca().spines['top'].set_visible(False)
