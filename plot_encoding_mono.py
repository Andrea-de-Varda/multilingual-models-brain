...
def load(model_prefix, monol = True, split_context = False, random = False, md = False, rh = False):
    if monol:
        if split_context:
            ...
            with open(f"results/monolingual_md_{model_prefix}", 'rb') as handle:
                file = pickle.load(handle) # MD network
        elif rh:
            with open(f"results/monolingual_rh_{model_prefix}", 'rb') as handle:
...
# def plot_md(MD, L, title, ylim=None, ylimstart=None, sig = [], name0 = "MD", name1 = "Language", color0 = "tomato", color1 = "navy", colname = "Network"):
#     MD[colname] = name0
#     L[colname] = name1
#     df_combined = pd.concat([MD, L], axis=0).reset_index(drop=True)
    
#     md_avg, md_se = MD["Score"].mean(), MD["Score"].std()/np.sqrt(len(MD))
#     l_avg, l_se = MD["Score"].mean(), MD["Score"].std()/np.sqrt(len(MD))
