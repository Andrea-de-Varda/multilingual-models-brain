...
def test_model_Ridge_sequential(X, y, n, saveto, save_results = True):
    kf = KFold(n_splits=n, shuffle=False, random_state = 0)
    out_reg = []
    out_coefs = []
    out_predictions = []
    ...
    if save_results:
        save(out_reg, f"results/out_reg/sequential_{saveto}") # saving all rs and coefficients for later use
        save(out_coefs, f"results/coefficients/sequential{saveto}")
    return np.mean(out_reg), np.std(out_reg)
    
def monolingual_encoding(langs, model_prefix, n_layers):
    layerwise_dict = {}
    if os.path.isfile(f"results/sequential/monolingual_{model_prefix}"):
        ...
        for n in range(n_layers+1):
            ...
            for idx, lang in enumerate(langs):
                the_r, the_sd = test_model_Ridge_sequential(fmri_data[idx], d[lang_code_dict[lang]], 10, saveto = f"{model_prefix}_{lang}_{n}")
                m.append(the_r)
                sd.append(the_sd)
            #########################
            mean_r = np.mean(m)
            ...
            layerwise_dict[n] = df
        save(layerwise_dict, f"results/sequential/monolingual_{model_prefix}")
    return layerwise_dict

def multilingual_encoding(langs, model_prefix, n_layers):
    layerwise_dict = {}
    if os.path.isfile(f"results/multilingual_{model_prefix}"):
        print(f"Encoding for {model_prefix} already done")
    else:
        for n in range(n_layers+1):
            print(f"Processing layer {n}")
            ...
            print(f"Mean r = {mean_r}")
        save(layerwise_dict, f"results/multilingual_{model_prefix}")
    return layerwise_dict
