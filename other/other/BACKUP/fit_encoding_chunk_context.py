...
def monolingual_encoding_chunked(langs, model_prefix, n_layers):
    layerwise_dict = {}
    if os.path.isfile(f"results/split_context/monolingual_{model_prefix}"):
        ...
        for layer_n in range(n_layers+1):
            ...
            for idx, lang in enumerate(langs):
                ...
                the_r, the_sd = np.mean(out_reg), np.std(out_reg)
                #print(f"{lang} = {the_r}")
                r_tot = round(pearsonr(out_pred, y_tot)[0], 4)
                print(f"{lang} = {r_tot}")
                out_predictions = [y_tot, out_pred]
                save(out_coefs, f"results/coefficients/chunked_{model_prefix}_{lang}_{layer_n}")
                save(out_predictions, f"results/predictions/chunked_{model_prefix}_{lang}_{layer_n}")
                m.append(the_r)
                sd.append(the_sd)
            #########################
            mean_r = np.mean(m)
